...
CMV::CMV()
{
	...
	for (TiXmlElement* elem = cock->FirstChildElement(); elem != NULL; elem = elem->NextSiblingElement())
	{
		...
		for (TiXmlElement* elem2 = elem->FirstChildElement(); elem2 != NULL; elem2 = elem2->NextSiblingElement())
		{
			...
			string amount = elem2->Attribute("amount");
			double amountd = ::atof(amount.c_str());
			c_load->push_amount(amountd);
...
CMV::~CMV()
{
	...
	if (cocktails.size() != 0)
		for (size_t i = 0; i < cocktails.size(); i++)
		{
			...
			for (size_t j = 0; j < cocktails.at(i)->get_ingredients().size() && j < cocktails.at(i)->get_amount().size(); j++)
			{
				...
				ct_s->SetAttribute("amount", cocktails.at(i)->get_amount().at(j));
			}
...
void CMV::push_Cocktail(Cocktail* c_h) {
...
void CMV::push_Dispenser(Dispenser* d_h) {
...
void CMV::push_Ingredient(Ingredient* i_g) {
